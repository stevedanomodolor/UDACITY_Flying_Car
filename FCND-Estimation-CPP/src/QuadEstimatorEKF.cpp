...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  Quaternion<float> q_int = q.IntegrateBodyRate(gyro, dtIMU);

  // convert quaternion back to euler 
  V3D euler = q_int.ToEulerYPR();
  float predictedRoll = euler.z;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  V3F accelInterialFrame = attitude.Rotate_BtoI(accel);
  // We are going to create an a matrix for only the x,y,z,x_dot,y_dot,z_dot states, we will just update the yaw state later because the integration is already done in the IMU update
  predictedState(0) = curState(0) + dt * curState(3);
  predictedState(1) = curState(1) + dt * curState(4);
  predictedState(2) = curState(2) + dt * curState(5);
  predictedState(3) = curState(3) + dt * accelInterialFrame.x;
  predictedState(4) = curState(4) + dt * accelInterialFrame.y;
  predictedState(5) = curState(5) + dt * (accelInterialFrame.z - CONST_GRAVITY);
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  // we will build the gPrime matrix
  gPrime(0, 3) = dt;
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
  gPrime(3, 6) = dt * (RbgPrime(0, 0) * accel.x + RbgPrime(0, 1) * accel.y + RbgPrime(0, 2) * accel.z);
  gPrime(4, 6) = dt * (RbgPrime(1, 0) * accel.x + RbgPrime(1, 1) * accel.y + RbgPrime(1, 2) * accel.z);
  gPrime(5, 6) = dt * (RbgPrime(2, 0) * accel.x + RbgPrime(2, 1) * accel.y + RbgPrime(2, 2) * accel.z);
